/**
 * Grab — stream assembler.
 *
 * Runs in an offscreen document so it can build Blobs and object URLs, which a
 * service worker cannot. Parses HLS playlists and DASH manifests, fetches the
 * segments, and concatenates them into one file.
 *
...
function report(jobId, phase, extra) {
...
async function buildTrack({ init, segments }, jobId, name, ext, progress) {
  ...
  await chrome.runtime.sendMessage({ type: 'assembled', jobId, blobUrl, name, ext });
  setTimeout(() => URL.revokeObjectURL(blobUrl), 10 * 60_000);
  return blob.size;
}

async function assemble({ jobId, kind, url, variant, name }) {
  cancelled.delete(jobId);
  report(jobId, 'preparing');
  ...
  report(jobId, 'done', { split });
}

/* --------------------------------------------------------------- messages */

const handlers = {
  async variants({ kind, url }) {
    if (kind === 'hls') {
      const text = await fetchText(url);
      if (!text.includes('#EXT-X-STREAM-INF')) return { variants: [] };
      const { variants } = parseMaster(text, url);
      return { variants: variants.map((v) => ({ id: v.url, label: labelFor(v), height: v.height })) };
    }
    if (kind === 'dash') {
      const reps = dashRepresentations(await fetchText(url), url)
        .filter((r) => r.kind === 'video')
  ...
  async assemble(msg) {
    assemble(msg).catch((e) => {
      const err = String(e?.message || e);
      report(msg.jobId, err === 'cancelled' ? 'cancelled' : 'error', { error: err });
    });
    return { started: true };
  },
